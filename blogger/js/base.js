var articleCatagorys = [{
        ...
        type: "dojoArticle",
        displayName: "Dojo Toolkit Tutorial",
        articles: [{
    ...
    articleBuilder = {
        ...
        articleTemplate: "<article><div class='articleImg'><a href='{{link}}'><img src='{{iamge}}'></a></div><div class='articleBody'><a href='{{link}}'><h1>{{heading}}</h1></a><p class='desc'><img class='showOnMobile' src='{{iamge}}'>{{desc}}</p></div></article>",
        getArticlesByTag: function(e, i) {
            ...
            for (o = this.articleCatagoryTemplate, o = o.replace("{{articleSectionHeading}}", i), a = 0; a < articleCatagorys.length; a++)
                for (l = articleCatagorys[a], t = 0; t < l.articles.length; t++) n = l.articles[t], n.tag.indexOf(e) > -1 && (r += this.buildArticleTemplate(n));
            o = o.replace("{{articles}}", r), document.getElementById("articlesContainer").innerHTML = o
        },
        ...
        getArticleByIds: function(e) {
            ...
            for (i = 0; i < articleCatagorys.length; i++)
                for (t = articleCatagorys[i], a = 0; a < t.articles.length; a++)
                    if (n = t.articles[a], n.id && n.id === e) return n;
            return o
        },
        buildArticlesById: function(e) {
            var i = this.getArticleByIds(e),
                a = "";
            return a += this.buildArticleTemplate(i)
        }
    },
    blogPageBuilder = {
        init: function() {
            this.encodeCode(), this.buildGoogleCodePrettifyScript(), this.buildSimilarArticle()
        },
        encodeCode: function() {
            var e = $("pre.encodeCodeblock");
            if (e) {
                var i = e.html();
                i && (i = i.replace(/<!--\$he/g, ""), i = i.replace(/\$he-->/g, ""), $("pre.encodeCodeblock").text(i))
            }
        },
        ...
        buildSimilarArticle: function() {
            $("section.similarArticle").each(function() {
                var e = articleBuilder.buildArticlesById($(this).attr("data-articles"));
                $(this).html(e)
            })
        }
    };
$(document).ready(function() {
    blogPageBuilder.init(), setTimeout(function() {
