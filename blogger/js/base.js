var articleCatagorys = [{
    ...
    type: "dojoArticle",
    displayName: "Dojo Toolkit Tutorial (AMD)",
    articles: [{
        heading: "Hello world application in DOJO and AMD foundation apis.",
        ...
        heading: "JAVA LIKE INHERITANCE IN JAVASCRIPT USING DOJO",
        ...
        heading: "Deep dive into MixIn based Inheritance in DOJO using declare and AMD concept",
...
var articleBuilder = {
    ...
    articleTemplate: "<article>" + "<div class='articleImg'>" + "<a href='{{link}}'>" + "<img src='{{iamge}}'>" + "</a>" + "</div>" + "<div class='articleBody'>" + "<a href='{{link}}'>" + "<h1>{{heading}}</h1>" + "</a>" + "<p class='desc'>{{desc}}</p>" + "</div>" + "</article>",
    getArticlesByTag: function(e, t) {
        var n, r, i = "",
            s, o, u;
        s = this.articleCatagoryTemplate;
        s = s.replace("{{articleSectionHeading}}", t);
        for (n = 0; n < articleCatagorys.length; n++) {
            u = articleCatagorys[n];
            for (r = 0; r < u.articles.length; r++) {
                o = u.articles[r];
                if (o.tag.indexOf(e) > -1) {
                    i += this.buildArticleTemplate(o)
                }
            }
        }
        s = s.replace("{{articles}}", i);
        document.getElementById("articlesContainer").innerHTML = s
    },
    buildArticleTemplate: function(e) {
        var t = this.articleTemplate;
        t = t.replace("{{iamge}}", "http://icodingclub.github.io/resource/image/" + e.image);
        t = t.replace("{{heading}}", e.heading);
        t = t.replace("{{desc}}", e.desc);
        t = t.replace("{{link}}", e.link).replace("{{link}}", e.link).replace("{{link}}", e.link);
        return t
    },
    ...
    buildArticlesById: function(e) {
        var t = this.getArticleByIds(e),
            n = "";
        n += this.buildArticleTemplate(t);
        return n
    }
};
var blogPageBuilder = {
    init: function() {
        this.encodeCode();
        this.buildGoogleCodePrettifyScript();
        this.buildSimilarArticle()
    },
    encodeCode: function() {
        var e = $("pre.encodeCodeblock");
        if (e) {
            var t = e.html();
            if (t) {
                t = t.replace(/<!--\$he/g, "");
                t = t.replace(/\$he-->/g, "");
                $("pre.encodeCodeblock").text(t)
            }
        }
    },
    ...
    buildSimilarArticle: function() {
        $("section.similarArticle").each(function(e) {
            var t = articleBuilder.buildArticlesById($(this).attr("data-articles"));
            $(this).html(t)
        })
    }
}
