  ...
  if(has("dom") && (has("dojo-inject-api") || has("dojo-dom-ready-api"))){
    ...
    if(has("dojo-inject-api")){
      ...
      req.injectUrl = function(url, callback, owner){
        ...
        node.src = url;
        // START QBO-PATCH
        // fixes the Cross Domain Script error reporting: https://jira.intuit.com/browse/QBO-18503

        if (req && req.baseUrl) {
          // req.baseUrl -> "http://cdn.trunk.dmostajo.qbo.intuit.net/c1/v73.116/scripts/dojo/dojo/"
          // slice the dojo/dojo off
          var scriptsUrl = req.baseUrl.slice(0, req.baseUrl.indexOf("scripts") + 7);  // "http://cdn.trunk.dmostajo.qbo.intuit.net/c1/v73.116/scripts"
          // and we only add crossorigin attribute if the url has the scriptsUrl in it
          if (url.indexOf(scriptsUrl) > -1) {
