...
return declare(null, {
	...
	//		Defines the algorithm used to distribute width changes when a column is resized.
	//		One of the following:
	//
	//		* default: The default algorithm does not resize any columns except for the
	//		  column being resized (and optionally the last column if `adjustLastColumn` is
	//		  true). It allows content to spill off beyond the right edge of the viewport.
	//		* proportional: This algorithm distributes the difference in column width across
	//		  all resizable columns in the grid to the right of the column that was resized.
	//		  This provides an intuitive-feeling resizing algorithm that ensures a
	//		  horizontal scrollbar never appears unless it is impossible to display all
	//		  content within the viewport.
	//		* proportional-all: This algorithm distributes the difference in column width
	//		  across all other resizable columns in the grid, ensuring that a horizontal
	//		  scrollbar never appears unless it is impossible to display all content
	//		  within the viewport. This algorithm ensures equal distribution at the expense
	//		  of user experience.
	...
	_resizeMouseUp: function(e){
		...
		// PATCH
		var newWidth = this._targetCell.offsetWidth + this._getResizeMouseLocation(e) - this._startX;
		if(newWidth < this.minWidth){
			//enforce minimum widths
			newWidth = this.minWidth;
		}
		this.proportionalResize(this._targetCell, newWidth);
	...
	// PATCH
	proportionalResize: function (cell, newWidth) {
		// Summary:
		//      called to resize a given cell to newWidth, resizing other cells
		//      based on resizeAlgorithm
		// cell: Element
		//      a grid header node
		// newWidth: String
		//      desired width of cell

		var delta = cell.offsetWidth ? newWidth - cell.offsetWidth : newWidth,
		...
		//PATCH
		if(this.resizeAlgorithm.indexOf("proportional") === 0){
			var columns = this.columns;
			var proportions = {};
			var widths = {};
			// The amount of space that needs to be filled by the resizable columns; subtract one extra pixel
			// to prevent rounding errors from generating scrollbars
			var availableWidth = this.headerNode.offsetWidth - newWidth - 1;
			// The amount of space the resizable columns currently use; used to determine the relative proportions
			// of the columns so the change can be distributed proportionally for each of the resizable columns
			var resizableWidth = 0;
			var colId;

			var foundResizedColumn = this.resizeAlgorithm === "proportional-all";
			obj.colNodes.forEach(function(node){
				if (!columns[node.columnId].hidden) {
					if(node.columnId === cell.columnId){
						foundResizedColumn = true;
					}else if(columns[node.columnId].resizable === false || !foundResizedColumn){
						availableWidth -= node.offsetWidth;
					}else if(foundResizedColumn){
						resizableWidth += widths[node.columnId] = node.offsetWidth;
					}
				}
			});
			...
			var remainder = 0,
				sumWidth = 0,
				adjustWidth = 0,
				numWidthsHigherThanMinWidth = 0;

			// PATCH
			// Make sure we still have available width to compute the proportions
			// and prevent the horizontal scrollbar showing
			if (availableWidth) {
				for(colId in widths){
					proportions[colId] = widths[colId] / resizableWidth;
					var colWidth = proportions[colId] * availableWidth;
					widths[colId] = Math.max(this.minWidth, colWidth);

					if (colWidth > this.minWidth) {
						remainder -= colWidth - widths[colId];
					}

					// get the sum to be used later
					sumWidth += widths[colId];
					// get number of columns whose width is greater than the minimum width
					if (widths[colId] > this.minWidth) {
						numWidthsHigherThanMinWidth++;
					}
				}

				// if the new computed width sum is greater than the available width, then the final resize
				// will cause it to overflow, readjust again
				if (sumWidth > availableWidth) {
					// redistribute the excess to columns that can take the widths (column width greater than min width)
					adjustWidth = (sumWidth - availableWidth) / numWidthsHigherThanMinWidth;
					for (colId in widths) {
						if (widths[colId] > this.minWidth) {
							widths[colId] -= adjustWidth;
						}
					}
				}
				...
				if(remainder){
					var lastRemainder;
					// small remainders get rounded to nothing anyway, so do not waste time spinning towards infinity;
					// also, if the remainder has not changed since last time, that means all resizable columns are already
					// as small as they can go
					while(remainder > 0.05 && remainder !== lastRemainder){
						lastRemainder = remainder;

						for(colId in widths){
							var oldWidth = widths[colId];
							var safeColDelta = Math.max(0, Math.min(lastRemainder * proportions[colId], oldWidth - this.minWidth));
							widths[colId] += safeColDelta;
							remainder -= safeColDelta;
						}
					}

					// if last column is resizable then apply a width
					if (columns[lastCol].resizable) {
						widths[lastCol] -= Math.max(0, Math.min(remainder, widths[lastCol] - this.minWidth));
					}
				}

				for(colId in widths){
					if (widths[colId]) {
						resizeColumnWidth(this, colId, widths[colId], typeof e !== "undefined" ? e.type : "mouseup", false);
					}
				}

				// if all are min widths, don't bother to resize the cell
				var allowNewWidthResize = !(Object.keys(widths).every(function(key) {
					return widths[key] === this.minWidth;
				}, this));

				if (allowNewWidthResize) {
					resizeColumnWidth(this, cell.columnId, newWidth, typeof e !== "undefined" ? e.type : "mouseup");
				}
			}
			// PATCH
		}else{
			...
			if(resizeColumnWidth(this, cell.columnId, newWidth, typeof e !== "undefined" ? e.type : "mouseup")){
				if(cell.columnId != lastCol && this.adjustLastColumn){

                    // PATCH we added this to get grid width
					if(this.adjustLastColumn){
						// For some reason, total column width needs to be 1 less than this
						gridWidth = this.headerNode.clientWidth - 1;
                    }
					...
					if(totalWidth + delta < gridWidth) {
						...
						resizeColumnWidth(this, lastCol, "auto", typeof e !== "undefined" ? e.type : "mouseup");
						...
						resizeColumnWidth(this, lastCol, this.minWidth, typeof e !== "undefined" ? e.type : "mouseup");
					}
				}
			}
		}
	...
	_getResizedColumnWidths: function (){
		...
		*/
		...
		var widths = {
			lastColId: colNodes[i-1].columnId,
			totalWidth: 0,
			colNodes: colNodes
		};
		// PATCH
		...
		while(i--){
			...
			widths.totalWidth += colNodes[i].offsetWidth;
			// PATCH
		}
